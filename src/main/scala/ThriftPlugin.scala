...
object ThriftPlugin extends Plugin {
  ...
  lazy val thriftSettings: Seq[Project.Setting[_]] = inConfig(Thrift)(Seq[Setting[_]](

    thrift := "thrift",

    thriftSourceDir <<= (sourceDirectory in Compile) {
      _ / "thrift"
    },
    ...
    thriftGenerate <<=  (
      streams,
      thriftSourceDir in Thrift,
      thriftOutputDir in Thrift,
      thrift in Thrift,
      thriftJavaOptions in Thrift,
      thriftJavaEnabled in Thrift,
      cacheDirectory
    ).map(thriftGenerateJavaTask),
    ...
    thriftGenerateJs <<= thriftGenerateJsTask,
    ...
    thriftGenerateRuby <<= thriftGenerateRubyTask,
    ...
    thriftGeneratePython <<= thriftGeneratePythonTask,
    ...
    watchSources <++= (thriftSourceDir) map {
      (tdir) => (tdir ** "*").get
    },
  ...
  def thriftGenerateJavaTask (streams: TaskStreams, sdir : File, odir : File, tbin: String, opts : Seq[String], enabled: Boolean, cache: File) = {
    println("Thrift Bin: " + tbin)
    if (enabled)
      compileThrift(sdir, odir, tbin, "java", opts, streams.log, cache / "thirft")
    else
      Seq[File]()
  }

  def thriftGenerateJsTask: Initialize[Task[Seq[File]]] =
    (streams, thriftSourceDir, thriftJsOutputDir, thrift, thriftJsOptions, thriftJsEnabled, cacheDirectory) map {
  ...
  def thriftGenerateRubyTask: Initialize[Task[Seq[File]]] =
    (streams, thriftSourceDir, thriftOutputDir, thrift, thriftRubyOptions, thriftRubyEnabled, cacheDirectory) map {
      (out, sdir, odir, tbin, opts, enabled, cache) =>
        if (enabled)
  ...
  def thriftGeneratePythonTask: Initialize[Task[Seq[File]]] =
    (streams, thriftSourceDir, thriftOutputDir, thrift, thriftPythonOptions, thriftPythonEnabled, cacheDirectory) map {
      (out, sdir, odir, tbin, opts, enabled, cache) =>
        if (enabled)
